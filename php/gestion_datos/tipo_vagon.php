<?php
    ...
    function getAllTipoVagon(){
        $query = "SELECT * FROM tipo_vagon ORDER BY nombre;";
        return consultarBBDD($query);
    }

    function getTipoVagonPorCodigo($codigo){
        $query = "SELECT * FROM tipo_vagon WHERE codigo = '" . $codigo . "';";
        return consultarBBDD($query);
    }
    ...
    function getTipoVagonPorAsientos($asientos_min = null, $asientos_max = null){
        if( $asientos_min && $asientos_min > 0 && $asientos_max && $asientos_max > 0 ){
            $query = "SELECT * FROM tipo_vagon WHERE asientos_max >= " . $asientos_min . " AND asientos_max <= " . $asientos_max . " ORDER BY nombre;";
        }elseif( $asientos_min && $asientos_min > 0 ){
            $query = "SELECT * FROM tipo_vagon WHERE asientos_max >= " . $asientos_min . " ORDER BY nombre;";
        }elseif( $asientos_max && $asientos_max > 0 ){
            $query = "SELECT * FROM tipo_vagon WHERE asientos_max <= " . $asientos_max . " ORDER BY nombre;";
        }else{
    ...
    function getTipoVagonPorCarga($carga_min = null, $carga_max = null){
        if( $carga_min && $carga_min > 0 && $carga_max && $carga_max > 0 ){
            $query = "SELECT * FROM tipo_vagon WHERE carga_max >= " . $carga_min . " AND carga_max <= " . $carga_max . " ORDER BY nombre;";
        }elseif( $carga_min && $carga_min > 0 ){
            $query = "SELECT * FROM tipo_vagon WHERE carga_max >= " . $carga_min . " ORDER BY nombre;";
        }elseif( $carga_max && $carga_max > 0 ){
            $query = "SELECT * FROM tipo_vagon WHERE carga_max <= " . $carga_max . " ORDER BY nombre;";
        }else{
            return getAllTipoVagon();
        }
        return consultarBBDD($query);
